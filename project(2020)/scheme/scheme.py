...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(proc, MacroProcedure):
            return scheme_eval(proc.apply_macro(rest, env), env)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        lambda_expressions = Pair(formals, body)
        procedure = do_lambda_form(lambda_expressions, env)
...
def do_and_form(expressions, env):
    ...
    if expressions == nil:
        return True
    curr_expr = expressions
    while curr_expr != nil:
        result = scheme_eval(curr_expr.first, env, True if curr_expr.rest == nil else False)
        if is_false_primitive(result):
            return False
        curr_expr = curr_expr.rest
    return result
    # END PROBLEM 12
