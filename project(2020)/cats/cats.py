...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    valid_paragraphs = [para for para in paragraphs if select(para)]
...
def accuracy(typed, reference):
    ...
    for i in range(typed_words_len):
        if i >= reference_words_len:
            break
        if typed_words[i] == reference_words[i]:
            count += 1
    return count / typed_words_len * 100
    # END PROBLEM 3
...
def time_per_word(times_per_player, words):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    times = [[p[i+1] - p[i] for i in range(len(p)-1)] for p in times_per_player]
...
def fastest_words(game):
    ...
    for word in word_indices:
        fastest_player = min(player_indices, key = lambda player: time(game, player, word))
        result[fastest_player].append(word_at(game, word))
    return result
    # END PROBLEM 10
