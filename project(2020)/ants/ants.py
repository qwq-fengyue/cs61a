...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        curr_place, cur_dist = self.place, 0
        while curr_place != beehive:
            if curr_place.bees and self.min_range <= cur_dist <= self.max_range:
                return rANTdom_else_none(curr_place.bees)
            curr_place, cur_dist = curr_place.entrance, cur_dist + 1
        return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if amount >= self.armor:
            fire_damage += self.damage
        bees_copy = self.place.bees[:]
        Ant.reduce_armor(self, amount)
        for bee in bees_copy:
            Insect.reduce_armor(bee, fire_damage)
...
# END Problem EC
    ...
    def reduce_armor(self, amount):
        ...
        if self.counter == 0 and self.armor <= 0:
            bees_win()
        # END Problem EC
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem Optional 2
        "*** YOUR CODE HERE ***"
        return self.contained_ant == None and not isinstance(other, ContainerAnt)
        # END Problem Optional 2
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem Optional 4
        "*** YOUR CODE HERE ***"
        if target and not target.scare_buffed:
            apply_status(make_scare, target, 2)
            target.scare_buffed = True
        # END Problem Optional 4
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        # BEGIN Problem Optional 5
        result = {}
        curr_place, curr_distance = self.place, 0
        while curr_place is not beehive:
            if curr_place.ant and curr_place.ant is not self:
                result[curr_place.ant] = curr_distance
            for bee in curr_place.bees:
                result[bee] = curr_distance
            curr_place, curr_distance = curr_place.entrance, curr_distance + 1
        return result
        # END Problem Optional 5
